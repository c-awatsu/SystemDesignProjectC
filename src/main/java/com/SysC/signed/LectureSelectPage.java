...
public class LectureSelectPage extends AbstractSignedPage{
	...
	public LectureSelectPage(){
		...
		List<String> gradeList = Arrays.asList("1年","2年","3年","4年");
		List<String> departmentList = Arrays.asList("バイオマテリアル学科","光システム学科","GSD学科");
		List<String> lectureFormat = Arrays.asList("必修","選択");

		//学年選択
		DropDownChoice<String> grade = new DropDownChoice<String>(
				"grade",new Model<String>(),gradeList);
		add(grade);

		//学科選択
		DropDownChoice<String> department = new DropDownChoice<String>(
				"subject",new Model<String>(),departmentList);
		add(department);
